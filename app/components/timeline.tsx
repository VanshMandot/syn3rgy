      ...
      <div className={`tl-hud${visible ? " on" : ""}`}>
        <div className="tl-hud-bar">
          ...
          <div className="tl-hud-c">
            <div className="tl-hud-badge hide-on-mobile">
  ...
  function RoundSection({ round, onVisibilityChange, pageReady }: RoundSectionProps) {
    ...
    const leftPanel = (
      ...
      >
        ...
        <div className={`tl-meta${metaIn ? " in" : ""}`}>
          <span className="tl-meta-code round-num">◈ ROUND_{round.roundNum}</span>
          <span className="tl-meta-sep" />
          <span className="tl-meta-code day-code">{round.dayCode}</span>
          
          <span className="tl-meta-sep hide-mobile" />
          <span className={`tl-dtag ${round.difficulty} hide-mobile`}>
            {round.difficulty.toUpperCase()}
          </span>
        </div>

        {/* TITLE with glitch layers */}
        <h2 className={`tl-round-title ${round.glowColor}${titleIn ? " in" : ""} hide-mobile`}>
          {round.title}
        ...
        {/* TAGLINE */}
        <p className={`tl-tagline${taglineIn ? " in" : ""} hide-mobile`}>{round.tagline}</p>
    ...
    const rightPanel = (
      ...
      >
        <h2 className={`tl-round-title ${round.glowColor}${titleIn ? " in" : ""} show-mobile-only`}>
          {round.title}
          <span className="g1" style={{ color: g1Color }} aria-hidden="true">
            {round.title}
          </span>
